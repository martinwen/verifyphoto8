...
import com.verify.photo.utils.DateUtil;
import com.verify.photo.utils.PreferenceUtils;
import com.verify.photo.utils.ViewUtil;
...
public class SwipeToLoadLayout extends ViewGroup {
    ...
    private static final int DEFAULT_REFRESH_COMPLETE_DELAY_DURATION = 0;
    ...
    private static final int DEFAULT_LOAD_MORE_COMPLETE_DELAY_DURATION = 0;

    private static final int DEFAULT_LOAD_MORE_COMPLETE_TO_DEFAULT_SCROLLING_DURATION = 0;
    ...
    protected void onFinishInflate() {
        ...
        if (mHeaderView != null) {
            time = (TextView) mHeaderView.findViewById(R.id.time);
        }
        setDayOrNight();
    }

    private void setDayOrNight() {
        info = (TextView) mHeaderView.findViewById(R.id.info);
        arrow = (ImageView) mHeaderView.findViewById(R.id.arrow);
        rl_bg = mHeaderView.findViewById(R.id.rl_bg);
        if (mFooterView != null) {
            tv_loadmore = (TextView) mFooterView.findViewById(R.id.tv_loadmore);
        }
        tv_time_info = (TextView) mHeaderView.findViewById(R.id.xlist_view_header_t);
...
//        }
    ...
    public void setHeadBackgroud(int color) {
        this.setBackgroundResource(color);
    }

//    public void toNight(){
//        info.setTextColor(getContext().getResources().getColor(R.color.header_textcolor_night));
//        time.setTextColor(0xffb7b7b7);
//        AnimationDrawable drawable = new AnimationDrawable();
//        drawable.addFrame(getContext().getResources().getDrawable(R.drawable.anim1),100);
//        drawable.addFrame(getContext().getResources().getDrawable(R.drawable.anim2),100);
//        drawable.addFrame(getContext().getResources().getDrawable(R.drawable.anim3),100);
//        arrow.setBackgroundDrawable(drawable);
//        if(mFooterView!=null){
//            tv_loadmore.setTextColor(0xffb7b7b7);
//        }
//        tv_time_info.setTextColor(0xffb7b7b7);
//      //  this.setBackgroundResource(R.color.list_bg_night);
//    }

    public void toDay() {
        info.setTextColor(getContext().getResources().getColor(R.color.header_textcolor_day));
        time.setTextColor(0xff131313);


//        arrow.setBackgroundDrawable(MyApplication.drawable);
        if (mFooterView != null) {
            tv_loadmore.setTextColor(0xff131313);
        }
        tv_time_info.setTextColor(0xff131313);
        //  this.setBackgroundResource(R.color.list_bg);
    }

    public void setUnique(String unique) {
        this.unique = unique;
        if (time != null) {
            time.setText(getLastTime(unique));
        }
    }

    public String getUnique() {
        return unique == null ? "" : this.unique;
    }

    public String getLastTime(String lab) {
        String time = getLastCurrentTime(lab);
        if (TextUtils.isEmpty(time)) {
            return "";
        }
        return changeDateFormat(Long.parseLong(time));
    }
     ...
     * @return 时间戳
     */
    public String getLastCurrentTime(String lab) {
        String time = new PreferenceUtils("Pull_List").getString(
                "last_refresh_currenttime_" + lab, "");
        return time;
    }
    ...
    public void setRefreshTime() {
        new PreferenceUtils("Pull_List").putString("last_refresh_currenttime_" + unique, String.valueOf(System.currentTimeMillis()));
        String date = changeDateFormat(System.currentTimeMillis());
        time.setText(date);
    }

    public void setRefreshTime(String lab) {
        Date date = DateUtil.getCurrentDate();
        new PreferenceUtils("Pull_List").putString("last_refresh_time_" + lab, DateUtil.dateToDateString(date));
        String lastTime = getLastTime(lab);
        Date datetime = DateUtil.getDate(lastTime);
        time.setText(DateUtil.dateToDateString(datetime, "MM-dd HH:mm"));
    }
    ...
    public String changeDateFormat(long time) {
        SimpleDateFormat format = new SimpleDateFormat(DateUtil.DATATIME_REFRESH);
    ...
    protected boolean canChildScrollUp() {
        if (android.os.Build.VERSION.SDK_INT < 14) {
            ...
            if (mTargetView instanceof RelativeLayout) {
                if (((RelativeLayout) mTargetView).getChildAt(1) instanceof RecyclerView) {
                    RecyclerView recyclerView = (RecyclerView) ((RelativeLayout) mTargetView).getChildAt(1);
                    return ViewCompat.canScrollVertically(recyclerView, -1);
                }
            }
            if (mTargetView instanceof RecyclerView) {//这样判断是为了当列表第一个position隐藏的时候 ViewCompat不起作用
    ...
    private void layoutChildren() {
        ...
        if (mHeaderView != null) {
            ...
            switch (mStyle) {
                case STYLE.CLASSIC:
                    // classic
                    int toptemp = paddingTop + lp.topMargin - mHeaderHeight + mHeaderOffset;
//                    L.e("toptemp"+toptemp);
//                    if(toptemp > 0 ){
//                        headerTop = 0;
//                    }else {
...
//                    }
        ...
        if (mTargetView != null) {
            ...
            switch (mStyle) {
                case STYLE.CLASSIC:
                    // classic
                    int toptemp = paddingTop + lp.topMargin + mTargetOffset;
//                    L.e("toptemp"+toptemp);
//                    if(toptemp > mHeaderHeight ){
//                        targetTop = mHeaderHeight;
//                    }else {
                    targetTop = toptemp;
//                    }
                    break;
        ...
        if (mFooterView != null) {
            ...
            if (mLoadMoreEnabled) {
                ...
                footerView.layout(footerLeft, height, footerRight, height + mFooterHeight);
            }
    ...
    private void scrollRefreshingToDefault() {
        if (isRecommend) {
            mAutoScroller.autoScrollToDefault();
        } else {
    ...
    RefreshCallback mRefreshCallback = new RefreshCallback() {
        ...
        public void onMove(int y, boolean isComplete, boolean automatic) {
            if (mHeaderView != null && mHeaderView instanceof SwipeTrigger && STATUS.isRefreshStatus(mStatus)) {
                if (isRecommend) {
                    if (isComplete) {
                        ViewUtil.setViewVisiblity(View.GONE, mHeaderView);
                    } else {
        ...
        public void onReset() {
            if (mHeaderView != null && mHeaderView instanceof SwipeTrigger && STATUS.isStatusDefault(mStatus)) {
                ...
                mHeaderView.setVisibility(GONE);
                if (isRecommend) {
                    if (mRefreshListener != null && mRefreshListener instanceof OnRefreshMineListener) {
                        ((OnRefreshMineListener) mRefreshListener).onComplete();
                    }
                }
            }
        }
